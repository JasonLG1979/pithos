...
class CellRendererAlbumArt(Gtk.CellRenderer):
    ...
    def update_icons(self, style_context):
        ...
        #fallback to get_background_color(Deprecated since version 3.16)
        #and get_color. *Most* new themes use 'theme_bg_color' and 'theme_fg_color'.
        #Some(older) themes use 'bg_color' and 'fg_color'.(like Ubuntu light themes)
        bg_bool, bg_color = style_context.lookup_color('theme_bg_color')
        if not bg_bool:
            bg_bool, bg_color = style_context.lookup_color('bg_color')
            if not bg_bool:
                bg_color = style_context.get_background_color(Gtk.StateFlags.NORMAL)
                logging.debug("Could not find 'theme_bg_color' or 'bg_color' with lookup_color.")

        fg_bool, fg_color = style_context.lookup_color('theme_fg_color')
        if not fg_bool:
            fg_bool, fg_color = style_context.lookup_color('fg_color')
            if not fg_bool:
                fg_color = style_context.get_color(Gtk.StateFlags.NORMAL)
                logging.debug("Could not find 'theme_fg_color' or 'fg_color' with lookup_color.")
...
class PithosWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, app, test_mode):
        ...
        self.prefs_dlg.password_entry.connect('grab-focus', self.on_password_entry_grab_focus)
    ...
    def style_context_change_handler(self, style_context):
        ...
        current_window_theme_name = current_theme.props.gtk_theme_name
        current_icon_theme_name = current_theme.props.gtk_icon_theme_name
        if (self.current_window_theme_name != current_window_theme_name
            or self.current_icon_theme_name != current_icon_theme_name):
            self.current_window_theme_name = current_window_theme_name
            self.current_icon_theme_name = current_icon_theme_name
            #Update rating icons and background, and generic cover icon and background.
            self.render_cover_art.update_icons(style_context)
            #If the current song is using the generic icon as the cover
            #we need to send a metadata-changed signal so MPRIS updates the generic icon cover also.
            if self.current_song is not None and self.songs_model[self.current_song_index][3] is None:
    ...
    def on_password_entry_grab_focus(self, *ignore):
        #clear out the password_entry and uncheck 
        #pandora_one_checkbutton when it's clicked into.
        self.prefs_dlg.pandora_one_checkbutton.set_active(False)
        self.prefs_dlg.password_entry.set_text('')

    def on_email_or_password_change(self, *ignore):
        #change state of explicit_content_filter_checkbutton dependant on password and email.
        if self.pandora.connected:
            new_email = self.prefs_dlg.email_entry.get_text()
            new_password = self.prefs_dlg.password_entry.get_text()
            ...
            if self.last_email != new_email or self.last_password != new_password: 
                ...
                self.prefs_dlg.pandora_one_checkbutton.set_active(self.last_one_checkbox)
                self.sync_explicit_content_filter_setting()
